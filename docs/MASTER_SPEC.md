...
- スタッフ: ダッシュボード / 技術者管理 / 案件管理 / アサイン・要員配置 / 案件オファー / 契約管理 / 工数・稼働実績 / 請求管理 / 取引先・営業 / マッチング検索（**ADMIN のみ**: 会社情報設定）
- エンジニア: マイページ のみ
...
- ※用語ルール（決定済み）: 本人画面では「オファー」ではなく「**案件一覧 / 案件回答**」と表記。管理側は「案件オファー」のまま。
