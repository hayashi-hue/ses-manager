...
ロジックは `src/lib/settlement.ts`（精算）・`src/lib/workflow.ts`（申請）・`src/lib/utils.ts`（表示）・`src/lib/invoice-*.ts`（請求書）・`src/lib/metrics.ts`（ダッシュボード指標：有給取得率・残業・単価据え置き）に集約。
...
- **追加KPI（`metrics.ts`）**:
  - **有給取得率**＝直近1年に承認された有給取得日数 ÷ 在籍社員（協力会社BP除く）の法定付与日数合計。付与は勤続年数ベース（0.5年=10/1.5年=11/2.5年=12/3.5年=14/4.5年=16/5.5年=18/6.5年〜=20日）。時間休は8時間=1日換算。
  - **平均残業時間**＝直近12ヶ月の工数から「実働 − 所定（その月の実営業日数×8時間）」の超過分を、1人月あたりで平均。
  - **平均単価**＝稼働中（ACTIVE/ORDERED）アサインの客先請求単価（月額）の平均。
- **単価据え置きピックアップ**: 実単価が**6ヶ月以上**変わっていない稼働中要員を一覧表示。要員ごとに契約タイムライン（適用開始日＝`startOn→signedOn→createdAt`）を遡り、現在契約の単価が連続する期間＝据え置き月数を算出。各行から契約編集（`/contracts/[id]/edit`）へ動線。
